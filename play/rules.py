...
SYSTEM_RULE_START_OF_GAME_CHECK = [
    ...
    (
        'And the game creates check_start_of_game_action',
        create_start_of_game_action_state,
    ),
]

def push_scan_hand_todo(game, todo, matched_line) -> List[Any]:
    ret = [line for line in todo]
...
SYSTEM_RULE_START_OF_GAME_SCAN_HAND = [
    ...
    (
        'Ane the scan is not done',
        lambda context: not any('scan_done' in line for line in context.todo),
    ),
    (
